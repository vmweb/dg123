...
<script>
	function startSubmitRefresh(){
		//dijit.byId('customersList<?php echo $this->customer['idNumber'];?>').set('content','document.EditCustomer<?php echo $this->customer['idNumber']; ?>.submit()');
        var dataElement = document.EditCustomer<?php echo $this->customer['idNumber']; ?>.submit();
        alert("startSubmitRefresh--1--");
        dijit.byId('customersList<?php echo $this->customer['idNumber'];?>').set('content',dataElement);
        alert("startSubmitRefresh--2--");
	}
</script>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" height="43px">
  <tr>
    <td width="100%" nowrap="nowrap" style="background-image: url(http://<?php echo $_SERVER['SERVER_NAME'];?>/vmweb2009R1/public/images/vmPcPics/toolBar/br.jpg); background-repeat:repeat-x;">       
		...
		><img  	onclick="startSubmitRefresh()"
