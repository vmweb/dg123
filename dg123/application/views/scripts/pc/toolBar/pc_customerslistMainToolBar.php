  ...
  <tr>
    <td width="100%" nowrap="nowrap" style="background-image: url(http://<?php echo $_SERVER['SERVER_NAME'];?>/vmweb2009R1/public/images/vmPcPics/toolBar/br.jpg); background-repeat:repeat-x;">       
		...
		<?php 
			...
			if ($curPage[$i+1] > 1) { ?>
			<img onclick="getCustomersListBefore();" src="http://<?php echo $_SERVER['SERVER_NAME'];?>/vmweb2009R1/public/images/vmPcPics/toolBar/vorheriger.jpg" style="cursor:pointer" alt="Vorheriger"	border="none"
		> <?php } else { ?>
		...
		<?php 
			$curPage = explode('/', $_SERVER['REQUEST_URI']);
			for ($i=1; count($curPage)>$i; $i++) { if ($curPage[$i]=='page') break; }
			if ($this->customers[0]['pages'] > $curPage[$i+1]) { ?>
		 	<img onclick="getCustomersListNext();" src="http://<?php echo $_SERVER['SERVER_NAME'];?>/vmweb2009R1/public/images/vmPcPics/toolBar/naechster.jpg" style="cursor:pointer" 	alt="N&auml;chster"	border="none"
		> <?php } else { ?>
		...
		><img onclick="searchCustomers();" src="http://<?php echo $_SERVER['SERVER_NAME'];?>/vmweb2009R1/public/images/vmPcPics/toolBar/suchen.jpg" style="cursor:pointer" alt="Suchen" border="none"
		...
		<table border="0">
			<tr align="right">
				<td nowrap="nowrap" align="right" valign="top" height="9px" style="margin:0 0 0 0; padding:0 0 0 0; spacing:0 0 0 0;">
					<input type="text" align="right" value="In der Liste: <?php echo $this->escape($this->customers[0]['recordCount']);?>" style="cursor:default;background-color:#F6F6F6;color:#444444;font-size:9px; margin:0 0 0 0; padding:0 0 0 0; spacing:0 0 0 0; border:0px; width:10em"  readonly="readonly" disabled="disabled">
				</td>
			</tr>
			<tr align="right">
				<td nowrap="nowrap" valign="top" height="9px" style="margin:0 0 0 0; padding:0 0 0 0; spacing:0 0 0 0;">
					<input id="currentPageCustomersList" align="right" type="text" style="cursor:default;background-color:#F6F6F6;margin:0 0 0 0; padding:0 0 0 0; spacing:0 0 0 0; border:0px;color:#444444;font-size:9px;width:10em"  readonly="readonly" disabled="disabled">
					<input id="maxPageCustomersList" value="<?php echo $this->escape($this->customers[0]['pages']);?>"  type="hidden">
				</td>
			</tr>
		</table>
	</td>
...
<script>
	function CounterUebertrag () {
		document.getElementById('currentPageCustomersList').value = 'Seite ' + document.getElementById('customersListCounter').value + ' von <?php echo $this->escape($this->customers[0]['pages']);?>';
	}
